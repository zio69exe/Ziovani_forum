...
<div class="card" style="background: #243642;  position: -webkit-sticky; position: sticky; top: 60px;">
    <div class="card-header">
    <h3 class="card-title text-light">Who to follow</h3>
    </div>
        <!-- /.card-header -->
        <div class="card-body p-0">
        <ul class="products-list product-list-in-card pl-3 pr-2">
            @foreach ($user as $users)
            <li class="item" style="background: #243642;">
            <div class="product-img">
                <img class="img-circle img-size-50" src="{{$users->profile->getAvatar()}}" alt="User Image">
            </div>
            <div class="product-info">
                <a href="#" class="product-title text-light">{{$users->profile->nama_lengkap}}</a>
                ...
                <p class="text-light">@ {{$users->name}}</p>
            </div>
            </li>
            @endforeach
